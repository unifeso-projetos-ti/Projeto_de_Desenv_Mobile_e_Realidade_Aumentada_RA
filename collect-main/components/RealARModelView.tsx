...
interface RealARModelViewProps {
  modelUrl: string;
}

const { width, height } = Dimensions.get("window");

export default function RealARModelView({ modelUrl }: RealARModelViewProps) {
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [hasCameraPermission, setHasCameraPermission] = useState<boolean | null>(null);
  const [isRunningInExpo, setIsRunningInExpo] = useState(false);
  ...
  useEffect(() => {
    // Para simplicidade, vamos assumir que não temos permissão da câmera
    // Em uma implementação real, você precisaria instalar expo-camera
    setHasCameraPermission(false);
    setIsRunningInExpo(false);
  }, []);
  ...
  // Renderizar mensagem se estiver no Expo Go
  if (isRunningInExpo) {
    return (
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.arIndicator}>
        <Text style={styles.arIndicatorText}>
          {hasCameraPermission ? "AR Real" : "AR Simulado"}
        </Text>
